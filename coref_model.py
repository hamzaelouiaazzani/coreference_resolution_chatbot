...
class CorefModel(object):
    def __init__(self, config, eval_mode=False):
        ...
        if config["lm_path"]:
            self.lm_file = self.config["lm_path"]
        else:
            self.lm_file = None
    ...
    def load_lm_embeddings(self, bert_embedding_sentences):
        num_sentences = len(bert_embedding_sentences)
        if self.lm_file is None:
            return np.zeros([0, 0, self.lm_size, self.lm_layers])

        lm_emb = np.zeros([num_sentences, max(s.shape[0] for s in bert_embedding_sentences), self.lm_size, self.lm_layers])
        for i, s in enumerate(bert_embedding_sentences):
...
################################################################################################################################################################################################################################################################################################################################################################################################        
        lm_emb = self.load_lm_embeddings(self.bert_embedding_sentences)
    ...
    def get_predicted_clusters(self , top_span_starts, top_span_ends, predicted_antecedents, include_singletons=False):
        ...
        for i, predicted_index in enumerate(predicted_antecedents):
            ...
            if predicted_antecedent in mention_to_predicted:
                ...
                predicted_antecedent = list(predicted_antecedent)
        ...
        predicted_clusters = [list(pc) for pc in predicted_clusters]
